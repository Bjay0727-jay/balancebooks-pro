...
export default function Recurring() {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between p-6 bg-gradient-to-r from-blue-600 via-[#1e3a5f] to-[#14b8a6]/50 rounded-2xl text-white shadow-xl">
        ...
        <button onClick={() => setModal('add-recurring')} className="flex items-center gap-2 px-4 py-3 bg-white text-[#14b8a6] rounded-xl font-medium hover:bg-blue-50 shadow-lg"><Plus size={18} />Add</button>
      </div>
          ...
          return (
            <div key={r.id} className={`flex items-center justify-between px-4 py-4 hover:bg-gradient-to-r hover:from-[#0f172a]/5/50 hover:to-[#14b8a6]/5/50 ${!r.active ? 'opacity-50' : ''}`}>
              <div className="flex items-center gap-3">
                ...
                <div><p className="font-medium text-slate-900">{r.name}</p><div className="flex items-center gap-2 text-xs text-slate-500"><span>{freq?.name}</span><span>Due: {r.dueDay}</span>{r.autoPay && <span className="text-[#14b8a6] font-medium">Auto-pay</span>}</div></div>
              </div>
              <div className="flex items-center gap-3">
                ...
                <button onClick={() => setEditRecurring(r)} className="p-2 rounded-lg hover:bg-[#14b8a6]/10 text-[#14b8a6]"><Edit2 size={14} /></button>
