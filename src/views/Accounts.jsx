...
const ACCOUNT_COLORS = ['#12233d', '#00b4d8', '#8b5cf6', '#f59e0b', '#ef4444', '#06b6d4', '#84cc16'];

function AccountForm({ account, onSubmit, onCancel }) {
  const [name, setName] = useState(account?.name || '');
  const [type, setType] = useState(account?.type || 'checking');
  const [icon, setIcon] = useState(account?.icon || '🏦');
  const [color, setColor] = useState(account?.color || '#12233d');
  ...
  return (
    <div className="space-y-4">
      <div>
        ...
        <input type="text" value={name} onChange={(e) => setName(e.target.value)} placeholder="e.g. Chase Checking" className="w-full px-4 py-3 border-2 border-slate-200 rounded-xl focus:ring-2 focus:ring-[#00b4d8] focus:outline-none" autoFocus />
      </div>
      <div>
        <label className="block text-sm text-slate-600 font-medium mb-2">Account Type</label>
        <div className="grid grid-cols-5 gap-2">
          {ACCOUNT_TYPES.map(at => (
            <button key={at.id} onClick={() => handleTypeChange(at.id)} className={`flex flex-col items-center p-2 rounded-xl border-2 transition-all ${type === at.id ? 'border-[#00b4d8] bg-[#00b4d8]/5' : 'border-slate-200 hover:border-slate-300'}`}>
      ...
      <div>
        ...
        <div className="relative">
          ...
          <input type="number" value={initialBalance} onChange={(e) => setInitialBalance(e.target.value)} className="w-full pl-10 pr-4 py-3 border-2 border-slate-200 rounded-xl focus:ring-2 focus:ring-[#00b4d8] focus:outline-none font-bold text-lg" />
        </div>
      </div>
      <div className="flex gap-3 pt-2">
        <button onClick={onCancel} className="flex-1 py-3 bg-slate-100 rounded-xl text-slate-700 font-medium">Cancel</button>
        <button onClick={() => { if (!name.trim()) return; onSubmit({ ...(account || {}), name: name.trim(), type, icon, color, initialBalance: parseFloat(initialBalance) || 0 }); }} disabled={!name.trim()} className="flex-1 py-3 bg-gradient-to-r from-[#12233d] to-[#00b4d8] text-white rounded-xl font-semibold shadow-lg disabled:opacity-50">
...
export default function Accounts() {
  ...
  return (
    <div className="space-y-6">
      {/* Total Balance */}
      <div className="p-6 rounded-2xl bg-gradient-to-r from-[#12233d] to-[#00b4d8] text-white shadow-lg">
            ...
            <div key={acct.id} className="bg-white rounded-2xl border-2 border-slate-100 shadow-sm p-5 hover:shadow-md transition-shadow">
              <div className="flex items-center justify-between mb-3">
                ...
                <div className="flex items-center gap-1">
                  <button onClick={() => setEditAcct(acct)} className="p-2 rounded-lg hover:bg-[#00b4d8]/10 text-slate-400 hover:text-[#00b4d8]"><Edit2 size={14} /></button>
                  {acct.id !== 'primary' && (
              ...
              <p className={`text-2xl font-bold ${balance >= 0 ? 'text-[#00b4d8]' : 'text-rose-600'}`}>{currency(balance)}</p>
            </div>
          );
        })}

        {/* Add Account Card */}
        <button onClick={() => setShowAddModal(true)} className="bg-gradient-to-r from-[#0a1628]/5 to-[#00b4d8]/5 rounded-2xl border-2 border-dashed border-[#12233d]/20 p-8 text-center hover:border-[#00b4d8] transition-all">
          <Plus className="mx-auto text-[#00b4d8] mb-2" size={24} />
      ...
      {/* Bank Connection */}
      <div className="p-6 rounded-2xl bg-gradient-to-r from-[#0a1628]/5 via-white to-[#00b4d8]/5 border-2 border-[#12233d]/20 shadow-sm">
        <div className="flex items-start gap-4">
          <div className="p-3 rounded-xl bg-gradient-to-br from-[#12233d] to-[#00b4d8] shadow-lg"><Shield size={24} className="text-white" /></div>
          ...
          <button onClick={() => setModal('connect')} className="px-4 py-2 bg-gradient-to-r from-[#12233d] to-[#00b4d8] text-white rounded-xl font-medium text-sm shadow-sm">
      ...
      {linkedAccounts.length > 0 && linkedAccounts.map(acc => (
        <div key={acc.id} className="bg-white rounded-2xl border-2 border-[#00b4d8]/20 shadow-sm p-6">
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-3">
              <div className="w-12 h-12 rounded-xl bg-gradient-to-br from-[#12233d] to-[#00b4d8] flex items-center justify-center text-white font-bold text-xl shadow-lg">{acc.institution.charAt(0)}</div>
              <div><h4 className="font-bold text-slate-900">{acc.institution}</h4><p className="text-xs text-[#00b4d8] font-medium">Auto-marking enabled</p></div>
            </div>
          ...
          <div className="grid grid-cols-2 gap-2">{acc.accounts.map(a => (
            <div key={a.id} className="p-3 rounded-xl bg-white border-2 border-[#12233d]/10"><p className="text-sm font-medium text-slate-700">{a.subtype}</p><p className="text-xs text-slate-400">{a.mask}</p></div>
          ))}</div>
        </div>
      ))}
