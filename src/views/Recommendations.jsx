                ...
                {rec.tips && rec.tips.length > 0 && (
                  <div className="mt-3 p-4 bg-gradient-to-r from-[#0f172a]/5 via-white to-[#14b8a6]/5 rounded-xl border border-[#1e3a5f]/10">
                    ...
                    <ul className="space-y-2">{rec.tips.map((tip, i) => (
                      <li key={i} className="text-sm text-slate-600 flex items-start gap-2"><span className="w-5 h-5 rounded-full bg-gradient-to-r from-[#1e3a5f] to-[#14b8a6] text-white text-xs flex items-center justify-center shrink-0 mt-0.5">{i + 1}</span>{tip}</li>
                    ))}</ul>
                  </div>
                )}
              </div>
