...
function EditableStatCard({ label, value, icon: Icon, iconBg, valueColor, onEdit }) {
  ...
  return (
    <div className="bg-white rounded-2xl p-5 border-2 border-[#1e3a5f]/20 shadow-sm cursor-pointer hover:border-[#14b8a6] hover:shadow-lg transition-all group" onClick={() => setModal(onEdit)}>
      <div className="flex items-center justify-between mb-3">
        <span className="text-slate-500 text-sm font-medium">{label}</span>
        <div className={`p-2 rounded-xl ${iconBg}`}><Icon size={16} className="text-[#14b8a6]" /></div>
      </div>
...
export default function Dashboard() {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-gradient-to-r from-blue-600 via-[#1e3a5f] to-[#14b8a6]/50 rounded-2xl p-6 text-white shadow-xl">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <div className="bg-white/15 backdrop-blur rounded-xl p-4 cursor-pointer hover:bg-white/25 transition-all border border-white/20" onClick={() => setModal('edit-beginning')}>
            <div className="flex items-center justify-between mb-1"><p className="text-[#14b8a6]/70 text-sm">Beginning Balance</p><Edit2 size={12} className="text-blue-200" /></div>
      ...
      {/* Stats Cards */}
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <EditableStatCard label="Beginning Balance" value={stats.beginning} icon={Wallet} iconBg="bg-gradient-to-br from-[#1e3a5f]/10 to-blue-200" valueColor="text-[#14b8a6]" onEdit="edit-beginning" />
        <EditableStatCard label="Income" value={stats.income} icon={ArrowUpRight} iconBg="bg-gradient-to-br from-green-100 to-green-200" valueColor="text-[#14b8a6]" onEdit="edit-income" />
        ...
        <EditableStatCard label="Ending Balance" value={stats.ending} icon={Target} iconBg="bg-gradient-to-br from-green-100 to-green-200" valueColor="text-[#14b8a6]" onEdit="edit-ending" />
      </div>

      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="bg-gradient-to-r from-[#0f172a]/5 via-white to-[#14b8a6]/5 rounded-2xl p-5 border-2 border-[#1e3a5f]/20 shadow-sm">
          <div className="flex items-center justify-between mb-3"><span className="text-slate-500 text-sm font-medium">Net Change</span><div className="p-2 rounded-xl bg-gradient-to-br from-[#1e3a5f]/10 to-[#14b8a6]/10"><TrendingUp size={16} className={stats.net >= 0 ? "text-[#14b8a6]" : "text-rose-600"} /></div></div>
          <p className={`font-bold text-2xl ${stats.net >= 0 ? 'text-[#14b8a6]' : 'text-rose-600'}`}>{stats.net >= 0 ? '+' : ''}{currency(stats.net)}</p>
        </div>
        <div className="bg-gradient-to-r from-[#0f172a]/5 via-white to-[#14b8a6]/5 rounded-2xl p-5 border-2 border-[#1e3a5f]/20 shadow-sm">
          <div className="flex items-center justify-between mb-3"><span className="text-slate-500 text-sm font-medium">Monthly Recurring</span><div className="p-2 rounded-xl bg-gradient-to-br from-[#1e3a5f]/10 to-blue-200"><RefreshCw size={16} className="text-[#14b8a6]" /></div></div>
          <p className="font-bold text-[#14b8a6] text-2xl">{currency(totalMonthlyRecurring)}</p><p className="text-xs text-slate-500 mt-1">/month committed</p>
        </div>
      </div>
      ...
      {savingsRecommendations.filter(r => r.priority === 'high').length > 0 && (
        <div className="bg-gradient-to-r from-[#14b8a6]/5 via-white to-blue-50 rounded-2xl p-6 border-2 border-[#14b8a6]/20 shadow-sm">
          ...
          <div className="space-y-3">{savingsRecommendations.filter(r => r.priority === 'high').slice(0, 2).map(rec => (
            <div key={rec.id} className="flex items-start gap-3 bg-white rounded-xl p-4 border border-[#14b8a6]/10 shadow-sm hover:shadow-md transition-all">
              <div className={`p-2 rounded-xl ${rec.type === 'alert' ? 'bg-gradient-to-br from-rose-100 to-rose-200' : rec.type === 'success' ? 'bg-gradient-to-br from-green-100 to-green-200' : 'bg-gradient-to-br from-[#1e3a5f]/10 to-[#14b8a6]/10'}`}>{rec.type === 'alert' ? <AlertTriangle size={16} className="text-rose-600" /> : <Lightbulb size={16} className="text-[#14b8a6]" />}</div>
              <div className="flex-1"><p className="font-medium text-slate-900">{rec.title}</p><p className="text-sm text-slate-500 line-clamp-2">{rec.description}</p>{rec.potential > 0 && <p className="text-sm text-[#14b8a6] font-semibold mt-1">Save up to {currency(rec.potential)}/mo</p>}</div>
            </div>
          ))}</div>
        </div>
      )}

      <div className="bg-white rounded-2xl p-6 border-2 border-[#1e3a5f]/10 shadow-sm">
        <h3 className="font-semibold text-slate-900 mb-4 flex items-center gap-2"><CreditCard size={18} className="text-[#14b8a6]" />Spending by Category</h3>
      ...
      <div className="bg-white rounded-2xl border-2 border-[#14b8a6]/10 shadow-sm overflow-hidden">
        ...
        <div className="divide-y divide-slate-100">{monthTx.slice(0, 6).map(tx => {
          ...
          return (
            <div key={tx.id} className="flex items-center justify-between px-6 py-4 hover:bg-gradient-to-r hover:from-[#0f172a]/5/30 hover:to-[#14b8a6]/5/30">
              ...
              <span className={`font-bold text-sm ${tx.amount > 0 ? 'text-[#14b8a6]' : 'text-slate-900'}`}>{tx.amount > 0 ? '+' : ''}{currency(tx.amount)}</span>
            </div>
          );
        })}</div>
      </div>
      ...
      <div className="bg-white rounded-2xl border-2 border-[#1e3a5f]/10 shadow-sm overflow-hidden">
        <div className="flex items-center justify-between p-4 border-b border-[#1e3a5f]/10 bg-gradient-to-r from-purple-50 via-white to-blue-50">
          <div className="flex gap-2">
            <button onClick={() => setDashboardWidget('ytd')} className={`px-4 py-2 rounded-lg font-medium text-sm transition-all ${dashboardWidget === 'ytd' ? 'bg-gradient-to-r from-purple-600 to-blue-600 text-white shadow-md' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'}`}>Year-to-Date</button>
            <button onClick={() => setDashboardWidget('cashflow')} className={`px-4 py-2 rounded-lg font-medium text-sm transition-all ${dashboardWidget === 'cashflow' ? 'bg-gradient-to-r from-purple-600 to-blue-600 text-white shadow-md' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'}`}>Cash Flow</button>
          </div>
