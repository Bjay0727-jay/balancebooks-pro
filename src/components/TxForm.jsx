  ...
  return (
    <form onSubmit={handle} className="space-y-4">
      <div>
        ...
        <input type="date" value={form.date} onChange={(e) => setForm({ ...form, date: e.target.value })} className="w-full px-4 py-3 bg-gradient-to-r from-[#0a1628]/5 to-white border-2 border-[#12233d]/20 rounded-xl focus:ring-2 focus:ring-[#00b4d8]" required />
      </div>
      <div>
        <label className="block text-sm text-slate-600 font-medium mb-2">Description</label>
        <input type="text" value={form.desc} onChange={(e) => setForm({ ...form, desc: e.target.value })} placeholder="Enter description" className="w-full px-4 py-3 bg-white border-2 border-[#12233d]/20 rounded-xl focus:ring-2 focus:ring-[#00b4d8]" required />
      </div>
      <div>
        <label className="block text-sm text-slate-600 font-medium mb-2">Amount</label>
        <input type="number" step="0.01" min="0" value={form.amount} onChange={(e) => setForm({ ...form, amount: e.target.value })} placeholder="0.00" className="w-full px-4 py-3 bg-gradient-to-r from-[#00b4d8]/5 to-white border-2 border-[#00b4d8]/20 rounded-xl focus:ring-2 focus:ring-[#00b4d8]" required />
      </div>
      <div>
        <label className="block text-sm text-slate-600 font-medium mb-2">Type</label>
        <select value={form.type} onChange={(e) => setForm({ ...form, type: e.target.value, category: e.target.value === 'income' ? 'income' : form.category, paid: e.target.value === 'income' ? true : form.paid })} className="w-full px-4 py-3 bg-white border-2 border-[#12233d]/20 rounded-xl focus:ring-2 focus:ring-[#00b4d8]">
      ...
      {!(splitMode && splits.length > 0) && (
        <div>
          ...
          <select value={form.category} onChange={(e) => setForm({ ...form, category: e.target.value })} className="w-full px-4 py-3 bg-white border-2 border-[#12233d]/20 rounded-xl focus:ring-2 focus:ring-[#00b4d8]">
          ...
          {splits.map((split, i) => (
            <div key={i} className="flex items-center gap-2">
              <select
                ...
                className="flex-1 px-3 py-2 bg-white border border-purple-200 rounded-lg text-sm focus:ring-2 focus:ring-[#00b4d8]"
              >
              ...
              <div className="relative w-28">
                ...
                <input
                  ...
                  className="w-full pl-7 pr-2 py-2 bg-white border border-purple-200 rounded-lg text-sm text-right font-medium focus:ring-2 focus:ring-[#00b4d8]"
                />
              </div>
      ...
      {showPaid && (
        <label className="flex items-center gap-3 cursor-pointer p-3 rounded-xl bg-gradient-to-r from-[#00b4d8]/5 to-blue-50 border-2 border-[#00b4d8]/20">
          <input type="checkbox" checked={form.paid} onChange={(e) => setForm({ ...form, paid: e.target.checked })} className="w-5 h-5 rounded border-green-300 text-[#00b4d8] focus:ring-[#00b4d8]" />
      ...
      <div className="flex gap-3 pt-4">
        ...
        <button type="submit" className="flex-1 px-4 py-3 bg-gradient-to-r from-[#12233d] to-[#00b4d8] text-white rounded-xl font-semibold shadow-lg hover:from-[#0a1628] hover:to-[#0096b7]">{tx ? 'Update' : 'Add'}</button>
      </div>
    </form>
  );
}
